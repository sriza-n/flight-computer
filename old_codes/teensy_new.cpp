...
// GPS variables
TinyGPSPlus gps;
float gpsLat = 27.680035f, gpsLng = 85.422318f;
int gpsSats = 0;
float gpsHdop = 0.0f;
bool gpsValid = false;
...
bool writeCSVFile()
{
  ...
  if (file.size() == 0)
  {
    ...
    file.println("SN,Time,Nano1,Nano2,Nano3,Nano4,ValveState,Current,Pressure,XPosition,YPosition,Altitude,EulerX,EulerY,EulerZ,AccelX,AccelY,AccelZ,GPSLat,GPSLng,GPSAlt,TeensyTemp");
  }
...
void task3()
{
  while (true)
  {
    while (Serial1.available() > 0)
    {
      ...
      {
        // Update global variables when new data is available
